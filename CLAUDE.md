...
| Pressure Advance | 3 styles: **Chevron tower** (per-layer PA); **Line** (garethky/K-factor) — a *generated tool path*, not sliced; **Pattern** (Ellis zigzag corner) — sliced, one specimen per PA value in a front-to-back grid | **Tower:** per-layer `M572 S` / `M900 K` / `SET_PRESSURE_ADVANCE`. **Pattern:** per-object PA via `CalibrationPAPostProcessor` (keyed on object-label name, gated on a first-layer marker; OctoPrint labeling forced); zigzag geometry `make_pa_zigzag_band` (built from `append_bead`). **Line** (`generate_line_pattern`): the dialog *generates the whole toolpath* (anchor bars + one constant-Y slow→fast→slow pass per PA with `M572`/`M900`/`SET_PRESSURE_ADVANCE` per line + boundary ticks + 7-segment PA number labels, relative E, E-rate from nozzle/filament/layer), writes it to a temp file, and loads a tiny **placeholder** STL. `CalibrationPALinePostProcessor` (`::builtin::pa_line_pattern?body=…`) then **splices** the toolpath over the placeholder's body (between the first post-marker `; printing object` / `; stop printing object`), keeping header/start/end. The slicer preview shows only the placeholder, so `generate_line_pattern` arms `Plater::set_pa_line_export_reminder(true)` → a one-shot "export to view" `RichMessageDialog` (with a "Don't show again" checkbox stored as AppConfig `hide_pa_line_export_reminder`) fires from `Plater::priv::on_process_completed`. Geometry: `make_pa_pattern` (tower chevron), `make_pa_zigzag_band` in `CalibrationModels.cpp` |
